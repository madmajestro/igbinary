	...
	do {
		...
		if (prototype_value == NULL) {
			...
			if (unmangled_class == NULL) {
				info = zend_hash_find_ptr(&ce->properties_info, key_str);
			} else {
				if (!strcmp(unmangled_class, "*")
				 || !strcasecmp(unmangled_class, ZSTR_VAL(ce->name))) {
					info = zend_hash_str_find_ptr(&ce->properties_info, unmangled_prop, unmangled_prop_len);
				}
			}
...
#endif
				zend_string_release(key_str);
				key_str = zend_string_copy(info->name);
